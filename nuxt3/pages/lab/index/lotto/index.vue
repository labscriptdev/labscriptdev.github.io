...
export default {
  meta: {
    ...
    source:
      "https://github.com/labscriptdev/labscriptdev.github.io/tree/main/pages/lab/index/lotto",
  },
  ...
  methods: {
    ...
    calculate() {
      ...
      const resultCombinations =
        factorial(play) / (factorial(small) * factorial(play - small));
      console.log(resultCombinations);
    },
  },
  computed: {
    lottoNumbers() {
      const initializeArrayWithRange = (start = 0, end = 10, step = 1) =>
        Array.from(
      ...
      let numbers = initializeArrayWithRange(
        this.lotto.numberStart,
        this.lotto.numberFinal
      ).map((number) => {
